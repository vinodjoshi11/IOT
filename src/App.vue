  ...
  <v-app>
    ...
    <template v-if="loggedIn"> 
      <v-toolbar app dark color="secondary"> 
          ...
          <v-spacer></v-spacer> 
           <v-toolbar-items dark color="secondary"class="hidden-sm-and-down"> 
            <v-btn flat v-for="item in items" :key="item.title" @click.native="clickMenu(item)" router> <v-icon v-if="activeMenuItem.indexOf(item.vertical) > -1" light v-html="item.icon" class="blue--text"></v-icon>
              <v-icon v-if="activeMenuItem.indexOf(item.vertical) < 0" dark v-html="item.icon"></v-icon>
              <v-list-tile-title v-text="item.title"></v-list-tile-title></v-btn> 
          </v-toolbar-items>
       <v-list class="pa-0 secondary" style="max-width: 15%;">
          <v-list-tile avatar tag="div">
            ...
            <v-menu bottom left offset-y origin="bottom right" transition="v-slide-y-transition">
              <v-btn icon light slot="activator">
                <v-icon class="whitefont">more_vert</v-icon>
              </v-btn>
              <v-list>
      ...
      <v-footer :fixed="fixed" app class="secondary">
        <v-flex xs12 style="text-align: center;"><span>&copy; Surmountsoft 2018 Crm </span>
        </v-flex>
      </v-footer>
