      ...
      <v-card>
       <v-card-text>
        <v-card-title>
          Devices
          <v-spacer></v-spacer>  
           <v-text-field append-icon="search" label="Search" single-line hide-details v-model="searchVm.contains.DeiceName" v-on:keyup="searchUsers"></v-text-field>
          &nbsp;
        </v-card-title>
      </v-card-text>  
        <v-data-table v-bind:headers="headers" v-bind:items="items" :search="search" v-bind:pagination.sync="pagination" hide-actions
          ...
          <template slot="items" slot-scope="props" class="body-2">
            <td class="body-2">{{ props.item.DeiceName }}</td>
  ...
  export default {
    ...
    methods: {
      ...
      remove(item) {
        ...
        if (rootComponent) {
          rootComponent.openDialog('Do you want to delete this item?', '', () => {
            this.api.deleteData('userrole/' + item.id.toString()).then((res) => {
              this.getdevices()
            }, (err) => {
              console.log(err)
            })
      ...
      changeStatus(item) {
        ...
        this.api.putData('userrole/' + item.id.toString(), item).then((res) => { }, (err) => {
          console.log(err)
        })
      },
      searchUsers() {
        ...
        this.api.getData('userrole?' + query).then((res) => {
          this.items = res.data
      ...
      getdevices() {
        this.api.getData('userrole').then((res) => {
          this.items = res.data
